---
title: "GSS Analysis about opinions on sexual behaviour in US from 1972 to 2016"
...
sub <- mutate(sub, 
              ...
              income = realinc)
```
I prepared the data by using the latest GSS panel data. There are three functions (adapted from Jonah Gabry's QMSS package) for time series analysis. Having extracted the relevant variables, I proceeded to remove those that were not relevant to the analysis i.e. those factors that represented "Others". I included as well as recoded several variables:  

**Year**: 31 years from 1972 to 2016  
**Homosex**: Recoded such that a score of 1 reflected a 'not at all wrong' 
**Conclerg**: Recoded such that a score of 1 reflect a 'none at all' response to the question, "I am going to name some institutions in this country. As far as the people running these institutions are concerned, would you say you have a great deal of confidence, only some confidence, or hardly any confidence at all in them? ". This is on a 4 point scale.   
**Sex**: Binary where 1 is female and 0 is men.   
**Age**: Unchanged.  
**Partyid**: A self-described strong republican is listed as 1 on an increasing scale that ends with a self-described strong democrat as 6.  
**Marital**: Never married as 1 and all other statuses including divorce or widowed as 0.   
**Educ**: Increasing from 0 to 20 for each increasing year of schooling.  
**Realinc**: Unchanged in real terms.  
**Race**: 1 for white and 0 for others.  
**Pray**: How often one prays a week, recoded with 1 as never and 6 as more than 1 a day.   
**Fund**: A variable that takes self-reported fundamentalism/liberalism of the religion. where 1 is fundementalist and others is 0.   
**Attend**: How often one attends religious services where never is 0 and 8 as several times as week. 
**Size**: Population size of area where respondent resides.   

For more specific references, one can obtain the codebook here [http://gss.norc.org/Documents/codebook/Panel%20Codebook.pdf].   
...
by.year.ts <- mutate(by.year.ts, 
                     ...
                     fund_pct=fund*100)
```
I then recoded several variables to obtain percentage terms. With female_pct, white_pct, ba_pct, married_pct and fund_pct representing the percentage of respondents for each survey wave. 

## Some trends from panel data
```{r}
# correlations
...
## Some basic regressions with plenty of problems
```{r}
...
```
There is indication that there is autocorrelation(1). It also probably fails the vif and is also a unit root process. 

## More problems but this time with a year trend
```{r}
# include year trend
lm2 <- update(lm1, ~ .  + year)
summary(lm2)
...
## Finally proceeds to first-difference the data to get rid of unit root process
```{r}
...
There is a clear, unambiguous downwards trend from around 1992 till today in the first-differenced data from the panel. The first-differenced percentage of white respondents, church attendance, size of the town and years are significant predictors.

Auto.arima did not suggest any alterations which is a little strange. 

Just getting started with this and there are probably alot of statistical errors in this which require further addressing. Let me know at [email](k.goh@columbia.edu). 
