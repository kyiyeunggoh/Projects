...
df_cdc_1[,-c(6,13,16,17,19)][df_cdc_1[, -c(6,13,16,17,19)] > 6] <- NA
...
#anyhow whack model (mental_14)

#reduced form model
df_red<-na.omit(df_cdc_1%>%
                  select("white","male","unhealth","working","child",ever_dep="ever_dep",educlvl="educ_lvl"
                         ,"income","smoke",billspay="bills_pay",moneyfood="money_food",feelmoney="feel_money","stress",
                         "city","race","age",ment14="mental_14"))
...
simple_model<-glm(ment14~as.factor(white)+as.factor(male)+as.factor(unhealth)+as.factor(working)+child+
                    ...
                    as.factor(city)+race+age,data=df_red)
...
mod_fit<-train(as.factor(ment14)~as.factor(white)+as.factor(male)+as.factor(unhealth)+as.factor(working)+child+
                 +educlvl+
                 income+smoke+billspay+moneyfood+feelmoney+stress+
                 as.factor(city)+race+age, data=df_train, 
...
table(pred)
ctable <- as.table(matrix(c(3043, 276, 73, 175), nrow = 2, byrow = TRUE))
fourfoldplot(ctable, color = c("#CC6669", "#99CC99"),
             conf.level = 0, margin = 1, main = "Confusion Matrix")

#randomForest
df_red<-na.omit(df_cdc_1%>%
                  select("white","male","unhealth","working","child",ever_dep="ever_dep",educlvl="educ_lvl","income","smoke",billspay="bills_pay",moneyfood="money_food",feelmoney="feel_money","stress",
                         "city","race","age",ment14="mental_14"))
  
#recode all to categorical 
df_red$city<-as.factor(df_red$city, levels=c("urban","suburban","rural"))
df_red$male<-as.factor(df_red$male, levels=c(0,1))
df_red$unhealth<-as.factor(df_red$unhealth,levels=c(0,1))
df_red$working<-as.factor(df_red$working,levels=c(0,1))
count(df_red$child)
as.df_red$child<-recode(df_red$child,"0='none';1='one';2='two';3='three';4='four';
                     5='five';6='six';7='seven';9='nine'")
df_red$child[df_red$child=='99'] <- NA
df_red$child<-as.factor(df_red$child)
df_red$ever_dep<-as.factor(df_red$ever_dep,levels=c(0,1))
...
df_red1<-df_red%>%
  select(-c("race","ever_dep"))
na.omit(df_red1)
df_red1$ever_dep<-as.factor(df_red1$ever_dep)
...
model11<-randomForest(ment14 ~ ., data = ment_train, ntree = 1000, mtry = 4, importance = TRUE)
model11
...
## RandomForest is best, stick with it 
